...
void AALSBaseCharacter::Tick(float DeltaTime)
{
	...
	else if (MovementState == EALSMovementState::InAir)
	{
		...
		if (bHasMovementInput && IsUseMantle)
		{
...
void AALSBaseCharacter::EventOnLanded()
{
	...
	if (bHasMovementInput && VelZ >= 600.0f)
	{
		OnBreakfall();
	}
	else
...
bool AALSBaseCharacter::IsSupportOverlayStateAiming(EALSOverlayState LocalOverlayState)
{
	switch (LocalOverlayState)
	{
...
bool AALSBaseCharacter::CanUseAiming()
{
	return IsSupportOverlayStateAiming(OverlayState);
}

bool AALSBaseCharacter::CanUseCombat()
{
	return IsSupportOverlayStateAiming(OverlayState) && RotationMode != EALSRotationMode::Aiming;
}
...
void AALSBaseCharacter::CombatModePressed()
{
	if (CanUseCombat())
	{
		SetRotationMode(EALSRotationMode::Combat);
	}
}

void AALSBaseCharacter::CombatModeReleased()
{
	if (CanUseCombat())
	{
		SetRotationMode(DesiredRotationMode);
